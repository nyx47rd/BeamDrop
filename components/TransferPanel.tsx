...
export const TransferPanel: React.FC<Props> = ({ 
  ...
  return (
    <div className="w-full flex flex-col h-full max-h-[80vh]">
      ...
      {progress && (
        <div className="w-full shrink-0 bg-[#171717] rounded-[2rem] p-6 mb-6 border border-[#262626] animate-in fade-in slide-in-from-bottom-2">
      ...
      {!progress && (
        <div className="flex-1 flex items-center justify-center mb-10 shrink-0">
            ...
            >
              ...
              <span className="text-[#a3a3a3] font-medium group-hover:text-white transition-colors">Send Files</span>
            </button>
        </div>
      )}
          ...
          <div className="flex-1 min-h-0 w-full flex flex-col">
              ...
              >
                  {receivedFiles.map((file, idx) => (
                      <div key={idx} className="w-full flex items-center justify-between p-4 bg-[#171717]/50 rounded-2xl border border-[#262626]/50 animate-in fade-in slide-in-from-bottom-2 duration-300">
                          <div className="flex items-center gap-3 overflow-hidden flex-1 mr-4">
                              <div className="w-8 h-8 rounded-full bg-[#262626] flex items-center justify-center shrink-0">
                                 <Check className="w-4 h-4 text-green-500" />
                              </div>
                              <span className="text-sm text-[#d4d4d4] truncate">{file.name}</span>
                          </div>
                          <a 
